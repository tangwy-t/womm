...
每枚徽章都属于三个稀有度之一，决定其视觉呈现：
- 🥉 **Bronze** — 最常见的成就
- 🥈 **Silver** — 中度挑战
- 🥇 **Gold** — 稀有、史诗级成就

默认使用 **`github` 模板**（纯图标正方形勋章，160×160，无文字），完美复刻 GitHub 官方成就风格。其他模板（badge/wide/terminal/stamp）显示为带文字的卡片，供不同场景使用。
...
- **`github`**(默认) — **纯图标正方形勋章，仿 GitHub 官方成就风格**（160×160，无文字，按 tier 着色：🥉 Bronze / 🥈 Silver / 🥇 Gold）
- `badge` — 横向紧凑
- `wide` — 宽幅卡片
- `terminal` — 终端输出模拟
- `stamp` — 圆形印章
